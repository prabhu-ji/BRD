...
import { DocumentTextIcon, ArrowDownTrayIcon, CheckCircleIcon, ExclamationCircleIcon } from '@heroicons/react/24/outline';
...
function GenerateBRD() {
  ...
  const startGenerationProcess = async (data) => {
    ...
    try {
      ...
      // In a real application, the files would be uploaded here
      // Since we can't access the original files from localStorage, 
      // we would need the user to re-upload them, or store them on the server
      // temporarily between steps.
      
      // This is a simulation for the demo
      
      // Make API call to generate BRD
      const response = await axios.post('/api/generate-brd', formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to generate BRD');
      }
    } catch (error) {
      console.error('Error generating BRD:', error);
      setStatus('error');
      setErrorMessage(error.message || 'Failed to generate BRD. Please try again.');
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card">
        ...
        {status === 'generating' && (
          <div className="py-8">
            <h2 className="text-xl font-semibold text-gray-800 mb-6">Generating Your BRD Document</h2>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
              <h3 className="text-blue-800 text-sm font-medium mb-2">Processing Steps:</h3>
            ...
            <div className="border border-gray-200 rounded-lg p-6 mb-6 bg-gray-50">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-medium text-gray-900">{generatedDoc.fileName}</h3>
                    <p className="text-sm text-gray-500">
                      Generated on {new Date(generatedDoc.timestamp).toLocaleString()}
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="mt-4 border-t border-gray-200 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <span className="text-gray-900">{brdData.template.templateName}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Outputs:</span>{' '}
                      <span className="text-gray-900">{brdData.outputs.length} sections</span>
                    </div>
                  </div>
                </div>
              )}
        ...
        {status === 'error' && (
          <div className="py-8 text-center">
            ...
            <div className="flex justify-center">
              ...
              {errorMessage !== 'No BRD data found. Please create a BRD first.' && (
                <button
                  onClick={() => {
                    if (brdData) {
                      setStatus('generating');

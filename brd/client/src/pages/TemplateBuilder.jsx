...
import { PlusIcon, TrashIcon, ArrowDownIcon, DocumentIcon, PhotoIcon, TableCellsIcon } from '@heroicons/react/24/outline';
...
function TemplateBuilder() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const inputsResponse = await fetch('http://localhost:5555/api/config/inputs');
  ...
  const saveTemplatesToServer = async (templatesData) => {
    try {
      const response = await fetch('http://localhost:5555/api/config/templates', {
        method: 'POST',
        headers: {
  ...
  const handleLoadTemplate = (template) => {
    ...
    // Update selected inputs based on the template's overview
    const templateInputKeys = new Set(template.overview.map(input => input.key));
    const availableInputKeys = new Set(Object.keys(availableInputs));
    
    const updatedSelectedInputs = new Set(
      [...availableInputKeys].filter(key => !templateInputKeys.has(key))
    );
            ...
            {templates.map(template => (
              <div key={template.id} className="border rounded-md p-3 bg-gray-50">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-medium text-blue-800">{template.templateName}</h3>
                ...
                <div className="text-sm text-gray-600">
                  <p>{template.overview.length} input fields</p>
                  <p>{Object.keys(template.outputs).length} output sections</p>
                </div>
              </div>
            ))}
          </div>

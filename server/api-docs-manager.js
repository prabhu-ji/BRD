...
class APIDocsManager {
    ...
    async getAPIContext(integrationDetails) {
        ...
        const relevantEndpoints = this.filterEndpointsByModules(
            docs.endpoints,
            modules
        );
        ...
        return endpoints.filter((endpoint) => {
            ...
            return modules.some((module) => {
                ...
                return (
                    ...
                    (pathLower.includes("employee") &&
                        moduleLower.includes("core")) ||
                    (pathLower.includes("payroll") &&
                        moduleLower.includes("payroll"))
                );
            });
        });
    }
    ...
    getModuleInfo(modules, docs) {
        ...
        modules.forEach((module) => {
            ...
            const moduleEndpoints = docs.endpoints.filter(
                (ep) =>
    ...
    getModuleDescription(module) {
        const descriptions = {
            ...
            "Employee Management":
                "Employee data management, profiles, organizational structure",
            Payroll: "Salary processing, tax calculations, payroll runs",
            Performance:
                "Performance reviews, goal setting, feedback management",
    ...
    formatForAI(apiContext) {
        if (!apiContext.hasDocumentation) {
            return `⚠️ No API documentation available for ${
                apiContext.vendor
            }. Consider adding documentation for more accurate BRD generation.\n\nRecommendations:\n${apiContext.recommendations.join(
                "\n"
            )}\n\n`;
        }
        ...
        // Authentication
        if (
            apiContext.authentication &&
            Object.keys(apiContext.authentication).length > 0
        ) {
        ...
        if (apiContext.endpoints.length > 0) {
            ...
            apiContext.endpoints.slice(0, 10).forEach((endpoint) => {
                // Limit to first 10
            ...
            if (apiContext.endpoints.length > 10) {
                formattedContext += `... and ${
                    apiContext.endpoints.length - 10
                } more endpoints\n`;
            }
            formattedContext += "\n";
        }
        ...
        try {
            ...
            if (fs.existsSync(detailedFilePath)) {
                ...
                if (detailedData.apis && detailedData.apis.length > 0) {
                    ...
                    console.log(
                        `📊 Loaded ${detailedData.apis.length} detailed Postman APIs for darwinbox`
                    );
                }
            }
            ...
            if (fs.existsSync(simpleFilePath)) {
                ...
                if (simpleData.apis && simpleData.apis.length > 0) {
                    console.log(
                        `📋 Found ${simpleData.apis.length} additional APIs in simple format`
                    );

                    // Merge simple data if detailed doesn't exist
                    if (!this.vendorDocs.has("darwinbox")) {
                        const convertedSimpleData =
                            this.convertScrapedData(simpleData);
            ...
            console.error(
                "⚠️ Error loading scraped Postman data:",
                error.message
            );
        }
    }

...
class ConfluenceGenerator {
    ...
    generateBRDPageId(brdData) {
        ...
        console.log(`🔍 Generated Page ID: ${pageId} (Session: ${sessionId})`);
    ...
    // NEW: Check if content is AI-generated based on structure or markers
    isAIGeneratedContent(value) {
        if (typeof value === "string") {
            // Check for common AI-generated content patterns
            return (
                value.includes("*   ") || // Bullet points
                value.includes("\n*") || // Bullet points on new lines
                value.includes("•") || // Bullet points
                (value.includes("This document") && value.includes("integration")) ||
                (value.includes("The") && value.includes("ensure") && value.length > 100)
            );
        }
        
        if (typeof value === "object" && value !== null) {
            // Check if it has the AI structure with type and content
    ...
    // NEW: Check if array data represents table structure
    isTableData(value) {
        if (!Array.isArray(value) || value.length === 0) {
            return false;
        }

        // Check if all items are objects with consistent keys (table rows)
        if (typeof value[0] === "object" && value[0] !== null && !Array.isArray(value[0])) {
            const firstKeys = Object.keys(value[0]);
            if (firstKeys.length > 1) {
                // Check if other items have similar structure
                return value.slice(1, 3).every(item => 
                    typeof item === "object" && 
                    item !== null && 
                    !Array.isArray(item) &&
        ...
        // Check if all items are arrays (table rows as arrays)
    ...
    // Enhanced content type detection - ENHANCED FOR AI DETECTION
    detectContentType(key, value) {
        // Simple log without verbose debugging
        ...
        if (typeof value === "string") {
            // Check for AI-generated content markers first
            if (this.isAIGeneratedContent(value)) {
                return "ai_text";
            }
        ...
        if (Array.isArray(value)) {
            ...
            if (this.isTableData(value)) return "table";
            return "list";
        }
        ...
        if (typeof value === "object" && value !== null) {
            ...
            // Check for AI-generated content in object
            if (value.content && this.isAIGeneratedContent(value.content)) {
                return "ai_text";
            }
            ...
            // Check for diagram structure (mermaid, graphviz, etc.)
            if (
                value.diagram ||
                value.mermaid ||
                value.graphviz ||
                (typeof value.content === "string" &&
                    (value.content.includes("graph") ||
                        value.content.includes("flowchart") ||
                        value.content.includes("digraph")))
            ) {
                return "diagram";
            }
    ...
    // Enhanced smart content generation - RESTRUCTURED FOR AI INTEGRATION
    generateSmartContent(key, value, contentType = null) {
        ...
        // Handle AI-generated content with type field
        if (
            typeof value === "object" &&
            value !== null &&
            ...
            return this.generateTextContentFromAI(key, value);
        }

        // Route to appropriate generator based on detected type
        switch (detectedType) {
            case "table":
...
<div class="panel">
    ...
    generateTextContentFromAI(key, content) {
        ...
        // Extract actual content from AI structure
        let actualContent = content;
        
        // Handle structured AI objects with type and content fields
        if (typeof content === "object" && content !== null) {
            if (content.hasOwnProperty("content")) {
                actualContent = content.content;
                console.log(`📄 Extracted content from AI structure for ${key}`);
            } else {
                // If it's an object but doesn't have content field, stringify it
                actualContent = JSON.stringify(content, null, 2);
                console.log(`⚠️ No content field found in AI structure for ${key}, using stringified object`);
            }
        }
        ...
        if (typeof actualContent === "string") {
            ...
            // Better object handling - should rarely hit this now
            textHtml += `<p>${this.escapeHtml(String(actualContent))}</p>\n`;
        }
...
<ac:structured-macro ac:name="expand">
    ...
    async performDuplicateCheck(brdData) {
        try {
            ...
            const searchQueries = [
                // Most specific search (only if all terms are valid)
                client && vendor && businessUseCase ? `"${client}" AND "${vendor}" AND "${businessUseCase}"` : null,
                // Broader searches (only if both terms are valid)
                client && vendor ? `"${client}" AND "${vendor}" AND "BRD"` : null,
                businessUseCase && client ? `"${businessUseCase}" AND "${client}" AND "BRD"` : null,
                vendor && client ? `"${vendor}" AND "${client}" AND "Integration"` : null,
            ].filter((query) => {
                // Only include non-null queries with all valid terms
                if (!query) return false;
                
                // Extract quoted terms and validate they're not empty
                const terms = query.match(/"([^"]+)"/g);
                return terms && terms.every((term) => {
                    const cleanTerm = term.slice(1, -1).trim(); // Remove quotes and trim
                    return cleanTerm.length > 0; // Must have content
                });
            });
    ...
    generatePageTitle(brdData) {
        ...
        console.log(`📋 Generated Page Title: ${title.substring(0, 80)}...`);
